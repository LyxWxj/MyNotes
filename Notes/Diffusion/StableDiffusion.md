...
- **编码器 (`Encoder` 类)**：一个逐步下采样的深度卷积网络。
  ...
  - 在网络的中间层（`self.mid`），还加入了**自注意力块（`AttnBlock`）**，用于捕捉长距离依赖关系。
...
> [!example] `Autoencoder` 核心方法
>
> **`encode` 方法**
> - **输入**: `img`，形状为 `[batch_size, img_channels, img_height, img_width]` 的图像张量。
> - **输出**: 一个 `GaussianDistribution` 对象，包含对潜在表示的**均值（mean）和对数方差（log_var）**。
>
> **`decode` 方法**
> - **输入**: `z`，形状为 `[batch_size, emb_channels, z_height, z_height]` 的潜在张量。
...
> [!example] `UNetModel` 的 `forward` 方法
>
> **输入**:
> 1. `x` (`torch.Tensor`): 当前时间步的**带噪潜在图像**，形状为 `[batch_size, in_channels, height, width]`。`in_channels` 通常是4。
> 2. `time_steps` (`torch.Tensor`): 一个标量张量，表示当前的**时间步**。
> 3. `cond` (`torch.Tensor`): 可选的**条件嵌入**（例如，来自CLIP文本编码器的嵌入）。
>
> **输出**:
> `torch.Tensor`: 形状为 `[batch_size, out_channels, height, width]` 的**预测噪声**。

---

## 💡 两者的核心差异与协作

> [!example] Autoencoder vs U-Net 对比
...
## 🧠 VAE 解码器为什么能"认识"去噪后的潜在表示？

> [!note] 核心原理：训练时的"对齐"
> VAE 的**解码器（Decoder）** 之所以能"认识"并正确重建这个由去噪过程得到的干净潜在表示，根本原因在于**训练时构建的"共享潜在空间"**。
>
...
1. **VAE 训练阶段**：
   ...
   - 编码器 `E` 将 `x` 压缩成一个潜在分布 `q(z|x)`，然后采样得到一个具体的潜在表示 `z`（例如 `4x64x64`）。
   - 解码器 `D` 将这个 `z` 重建回图像 `x'`。
   - 训练目标：让 `x'` 尽可能接近 `x`（重建损失），同时让 `q(z|x)` 接近标准正态分布（KL 散度）。

   > [!tip] 训练结果
   > 解码器 `D` 学到了一个映射：**"任何来自这个潜在分布 `q(z|x)` 的 `z`，都应该解码成看起来像真实图像的东西。"**
...
2. **扩散模型训练阶段**：
   ...
   > [!tip] 训练结果
   > U-Net 学习生成的 `z_0` 的分布，与 VAE 编码器产生的 `z` 的分布**保持一致**。
...
> [!abstract] 无缝衔接
> 整个流程是**无缝衔接**的：**扩散模型负责在潜在空间中找到好的坐标点，VAE 解码器负责将这个坐标点渲染成图像**。两者通过共享同一个潜在空间的定义而协同工作。
...
> [!note] 功能
> 接收原始图像（如 `3x512x512`），通过逐步下采样，将其压缩成一个**高斯分布**（由均值和方差表示）。
...
> [!note] 功能
> 接收采样得到的潜在表示（如 `4x64x64`），通过逐步上采样，将其重建为像素空间的图像（如 `3x512x512`）。
