...
> [!cite] 来源
> 知乎专栏《VAE、Diffusion、Flow Matching 系统讲解（一）：VAE 变分自编码器》- 冰锐
...
> [!info] 流形假设
> 高维数据的实际自由度远低于维度。例如 28×28 的手写数字（784 维），变化因素只有笔画粗细、倾斜角度等约 10-20 个因素。VAE 用一个低维向量 $z$（如 20 维）表示这些潜在因素，并且认为 z 服从标准正态分布（先验假设）。
...
> [!important] VAE 的解法——变分推断
> 既然真实后验 $p(z|x)$ 算不出来，就训练一个 Encoder 网络 $q_\phi(z|x)$（输出高斯分布的参数 $\mu, \sigma$）去近似它。然后优化一个可计算的下界 ELBO 来替代不可解的积分。
>
...
> [!question] 为什么用 $\log$ 而非 $p(x)$？
> 1. 数值稳定——$p(x)$ 在高维空间极小（如 $10^{-300}$），取 log 后正常
> 2. 乘法变加法——联合似然 $\prod p(x_i)$ 取 log 后变为 $\sum \log p(x_i)$
> 3. $\log$ 单调递增，最优解等价
...
> [!tip] 为什么重建损失是 MSE？
> 假设 $p_\theta(x|z) = \mathcal{N}(x; \mu_\theta(z), \sigma^2 I)$，则 $-\log p_\theta(x|z) = \frac{\|x - \mu_\theta(z)\|^2}{2\sigma^2} + \text{const}$，忽略常数后就是 MSE。
...
> [!warning] 问题
> 从 $q_\phi(z|x) = \mathcal{N}(\mu_\phi(x), \sigma^2_\phi(x))$ 中采样 $z$ 的操作不可微，梯度无法从 Decoder 流回 Encoder。

> [!tip] 解决方案：把随机性 " 外包 " 给外部噪声
>
...
> [!tip] 缓解方法
> KL 退火、$\beta$-VAE（调节 KL 权重）、Free bits（给每维 KL 设最小值）。
...
> [!abstract] 一句话总结
> 真实后验 $p(z|x)$ 算不出来 → 用 Encoder $q_\phi(z|x)$ 近似 → 优化 ELBO（重建 + KL）作为替代目标。
...
> [!tip] 与下一章的衔接
> VAE 一步跳太远（从 $z$ 直接到 $x$），导致模糊。Diffusion 的策略是 " 把一步大跳拆成多步小走 "——逐步去噪，每步只需做微小调整。
