...
- **问题**：无法满足实时 SLO
**四个关键挑战：**
- **未能满足实时 SLO**：最先进的视频扩散模型（如 WAN) 及其高效变体是为了离线生成吞吐量而优化的，通过每次前向传播处理固定的大输入。这种大块输出违反了实时服务级别的目标，后者呀求最小化首帧时间 (TTFT) 和严格的每帧截止时间。固定的输入大小也阻止了在不同硬件负载下的自适应调度，使得在异构 GPU 环境中满足 SLO 变得困难。
- **无界时间范围内的漂移：** 现有的视频流水线使用针对有限时间上下文校准的静态配置来生成固定长度的片段。这些假设在连续的直播流中失效，因为时间上下文，内容统计和用户提示都在动态演变。在长时间范围内，这些静态组件会累计错位，导致视觉漂移和时间连贯性下降。
- **高速动态下的运动撕裂**：当前的视频扩散模型主要是在慢动作数据集上训练的，使他们难以适应快速运动。它们的推理流水线通常用大的 Chunk 大小和基于规则的噪声调度来抑制帧间变化，这会导致运动过度平滑并侵蚀细节。因此，以快速摄像机运动或运动序列为特征的直播流会遭受模糊，重影和运动撕裂。
- **GPU 扩展性差**：随着 AI 驱动的直播从个人创作者扩展到企业级部署，多 GPU 扩展变得至关重要。然而，现有的离线扩散并行化策略不能直接用于实时工作负载。序列并行性，会受到不可预测的通信开销的影响，而朴素的流水线并行性在 FPS 提升上效果甚微。由于每帧延迟约束在实时工作负载中占主导地位，实现可扩展，低抖动的性能仍然是一个开放的挑战。
...
为了在最大化 GPU 利用率的同时满足 SLO,我们需要动态调整 Batch 大小。

> 给定目标帧率 $f_{SLO}$ ,Chunk 大小为$T$，也就是每次迭代处理$T$帧，整体推理延迟取决于 Chunk 大小和批大小$L(T,B)$,为了确保实时处理，B 和 T 的乘积$B*T$ 不能超过已从输入流中收集的帧数。
> 推理延迟可以近似为：
>
> $$

L(T,B) = \frac{A(T,B) + P_{model}}{\eta * BW_{HBM}}

$$
> 其中$A(T,B)$和$P_{model}$表示激活内存占用和模型参数的内存容量，$\eta * BW_{HBM}$是有效内存带宽。
> 结合FlashAttention,激活项$A(T,B)$与$BT$成线性比例，导致延迟$L(T,B)$也呈比例增长，因此实际的帧率$f = \frac{(B*T)}{L(T,B)} ∝ \frac{B}{1+B}$
> 随着批大小 B 的增加而增加，因为 GPU 利用率提高了。当系统接近 Roofline 模型的拐点（标志着从内存受限到计算受限的转变）时，调度器自适应地收敛到一个最优批大小 B*，以最大化吞吐效率。
**批调度策略**：
