...
| 符号 | 含义 | 类比 |
|------|------|------|
| $p_\theta$ | 参数化概率分布，$\theta$ 为神经网络参数 | "学习得到的模拟器" |
| $s_{t+1}$ | 下一时刻**隐状态**（环境完整状态，通常不可直接观测） | 迷宫中的真实位置 |
| $o_{t+1}$ | 下一时刻**观测**（智能体实际"看到"的） | 你看到的画面 |
| $r_t$ | 当前步**奖励**（环境返回的标量信号） | 游戏得分 |
| $s_t$ | 当前隐状态 | 你此刻的真实位置 |
| $a_t$ | 当前动作 | 你按的方向键 |

**读法**："给定当前状态 $s_t$ 和动作 $a_t$，预测下一状态、下一观测和奖励的联合概率分布。"

在部分可观测设置中，模型必须从历史推断潜在信念状态：

$$h_t = (o_{\leq t}, a_{<t}) \quad \Rightarrow \quad z_t = q_\phi(z_t | h_t)$$

**📖 部分可观测详解**

- $h_t$：**历史/信念状态**——智能体积累的所有信息
- $o_{\leq t}$：从开始到 $t$ 的**所有观测**（$o_1, o_2, ..., o_t$）
- $a_{<t}$：$t$ **之前**的所有动作（$a_1, a_2, ..., a_{t-1}$）
- $z_t$：学到的**潜在状态表示**——从历史中"猜测"环境真实状态

**直觉**：你看不到完整地图，但通过走过的路和做过的动作，脑中建立起"内部地图"。

**📖 三种核心组件的数学对应**

| 组件 | 数学形式 | 作用 |
|------|---------|------|
| 编码器 | $z_t = q_\phi(z_t \mid o_{\leq t}, a_{<t})$ | 从历史推断潜在状态 |
| 动力学模型 | $\tilde{z}_{t+1} = p_\theta(\tilde{z}_{t+1} \mid z_t, a_t)$ | 预测下一潜在状态 |
| 解码器 | $\tilde{o}_{t+1} = p_\psi(\tilde{o}_{t+1} \mid z_t, \tilde{z}_{t+1})$ | 从潜在状态重建观测 |

注：$\tilde{z}$（波浪线）表示**预测值**，区别于编码器的真实输出。
...
世界模型利用学到的动力学做决策的**两种模式**：

**在线规划 (Online Planning)**——推理时实时搜索最优动作
```
当前状态 s_t
    ├→ 世界模型想象：走 a₁ → ... → 评估好坏
    ├→ 世界模型想象：走 a₂ → ... → 评估好坏  ← 选这个
...
```
- 类比：下棋时每一步都认真想棋
- 代表：MuZero（MCTS 搜索 1000+ 条轨迹）、PlaNet、TD-MPC
- 要求：实时（毫秒级决策）

**潜在想象 (Latent Imagination)**——训练时在脑子里"练功"
```
真实经验（少量）→ 训练世界模型 → 想象经验（大量）→ 训练策略
                                  ↓
                          在潜在空间 z_t 中展开 T=15 步
                          （不重建像素，比真实交互快几个数量级）
```
- 类比：平时在脑子里反复演练棋局
- 代表：Dreamer 系列
- DreamerV3：想象训练 1000 步 ≈ 真实环境 100 步，但快得多
