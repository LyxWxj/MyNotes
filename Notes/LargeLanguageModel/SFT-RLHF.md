...
- Data Mixture of Decay Stage: 融合Instruction Tuning的数据到其中。
...
首先一些人（众包）针对output pairs按照某些标准打分，得到大量有分数的output pairs.
通过对比output pairs和分数训练奖励模型，让奖 励模型对LM的输出进行打分，然后训练。
...
$\pi^{RL}_\phi$是学习到的RL策略, $\pi^{SFT}$是监督训练后的模型，$D_{pretrain}$是预训练分布，$\beta$是KL散度奖励系数，预训练损失系数$\gamma$，他们分别控制了KL惩罚与预训练梯度。
在PPO中，$\gamma$为0，也就是
$$
objective(\phi)=E_{(x,y)\sim D_{\pi RL}}[r_\theta(x,y)-\beta \log(\pi^{RL}_\phi(y|x)/\pi^{SFT}(y|x))]
$$
只保留策略散度，这一项限制了RL模型的输出不会离SFT的输出分布太远。
...
我们希望使用上方已经训练好的奖励模型去训练可以生成高质量内容的策略。
我们将奖励模型的输出作为我们使用PPO算法最大化对于所有LM输出的奖励（逐个Token）。
$$
...
\nabla_\theta J(\theta) = \nabla_\theta (\sum_z \pi_\theta(z) R(z))\\
=\sum_z \nabla_\theta (\pi_\theta(z) )R(z)\\
\nabla_\theta \log \pi = \frac{\nabla_\theta \pi}{\pi}\\
\nabla_\theta \pi_\theta(z) = \pi_\theta(z)\nabla_\theta \log \pi_\theta(z)\\
\nabla_\theta E_{p_\theta}(R(z)) = \sum_z \pi_\theta(z)\nabla_\theta \log \pi_\theta(z) R(z)\\
= \sum_z \pi_\theta(z)R(z)\nabla_\theta \log \pi_\theta(z)\\
= E_{p_\theta}[R(z)\nabla_\theta \log \pi_\theta(z)]
\end{aligned}
$$
...
在PPO的上下文中，这个没有下标、带括号的 $r(\theta)$ 特指 **当前策略** 与 **旧策略** 在同一动作上的**概率比值**：
...
这是你笔记里 **Reward Model（奖励模型）** 输出的标量值，那个下标 $\theta$ 是**奖励模型自身的参数**（通常在RLHF阶段被冻结）。
...
你问的第二个符号 $\hat{A}$ 是强化学习里的核心概念 **Advantage（优势）**。
...
- **在RLHF中的落地**：在大模型微调里，我们通常不单独估算 $V(s)$，而是利用**序列级奖励**直接计算优势。但逻辑不变：$\hat{A}$ 决定了参数更新的**方向和力度**。
...
- **看到 $r(\theta)$（无下标、带括号）**：想**比例**。这是PPO特有的**截断对象**。
...
Assume that the policy $\pi$ is the set of all policies(nonparaetric assumption).
The maximizer is :
$$
...
我们从 **“RLHF 太重”** 的痛点切入，把 DPO 的逻辑、推导和直觉彻底理一遍。
...
- $r_\phi$ 是奖励模型给出的分数。
- $\beta$ 控制“听话程度”（KL 惩罚系数）。
- $\pi_{\text{ref}}$ 是冻结的初始模型（防止跑偏）。
...
**这个公式的意思**：
> 最优策略 $\pi^*$ 的输出概率，正比于参考策略的概率 $\times$ 奖励值的指数放大。
