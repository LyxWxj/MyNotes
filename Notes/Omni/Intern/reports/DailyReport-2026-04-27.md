...
1. **文献精读**：精读了 Qwen-Image 和 Qwen3-Omni 的技术报告，以及 vLLM-Omni 论文。Qwen-Image 的推理链路是 VAE Encoder → Qwen2.5-VL → MMDiT → VAE Decoder，各组件参数规模从 54M 到 20B 不等，耗时分布差异很大。Qwen3-Omni 采用 Thinker-Talker 流水线，Thinker 负责融合多模态输入，Talker 负责生成文本和音频码流。vLLM-Omni 在这类 AR 模型上已经做了 stage 分离和 prefill-decode disaggregation。
