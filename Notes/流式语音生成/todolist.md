...
- [ ] **论文阅读**
  - [x] [Duplex TTS 论文](https://arxiv.org/pdf/2602.02204) - 双工语音合成的理论基础
  - [x] [Google Slides 演示](https://docs.google.com/presentation/d/111-L8zF7A1j_YI_cR8JsblofdScdRr2f/edit#slide=id.p88) - Think Machine Lab 的 Duplex 架构设计
  - [x] 相关背景论文：StreamSpeech、SpeechGPT、VALL-E X

- [x] **社区 RFC 分析**
  - [x] [Omni RFC #3745](https://github.com/vllm-project/vllm-omni/issues/3745) - 仔细阅读 RFC 内容
  - [x] 分析社区对 Duplex 系统的设计讨论
  - [x] 梳理 RFC 中的关键技术点和争议问题

### Duplex 架构理解

- [x] **双工模型组成**
  - [x] s2t (Speech-to-Text) 模块：实时语音识别
  - [x] tts (Text-to-Speech) 模块：实时语音合成
  - [x] 双工并行机制：如何实现同时收听和说话？

- [x] **Agent Streaming vs Omni Duplex**
  - [x] Agent Streaming：传统串行流程（function calling → s2t → tts）
  - [x] Omni Duplex：原子性服务能力，支持并行处理
  - [x] 对比分析：延迟、吞吐、复杂度
...
- [x] **Thinker-Talker 并行机制**
  - [x] 分析 Thinker 和 Talker 如何并行执行
  - [x] 理解 hidden states 的流式传递
  - [x] 关注 `minicpmo_4_5_omni.py` 中的状态管理

- [x] **Session 状态管理**
  - [x] 会话级别的上下文维护
  - [x] 多阶段状态流转（Listening → Thinking → Speaking）
  - [x] 状态切换的触发条件和时机

- [x] **接口转换 Realtime**
  - [x] 理解如何将非实时模型转换为实时接口
  - [x] 分析音频 buffer 和 embedding cache 的作用
  - [x] 关注 prequery 等音频特殊逻辑
...
- [x] **context-chunk 处理**
  - [x] 滑动窗口机制：如何处理长序列音频
  - [x] 上下文信息：上一个窗口的 ASR/ST 文本、TTFF
  - [x] context 信息的传递和复用

- [x] **request-chunk 调度**
  - [x] 小请求的调度逻辑
  - [x] Session 绑定策略：同一 session 的请求路由
  - [x] 目标函数优化：延迟 vs 吞吐的平衡

### 系统侧优化

- [x] **Async CPU Zero-overhead**
  - [x] 分析语音模型的 CPU 开销来源
  - [x] 理解异步 CPU 处理的实现方式
  - [x] 识别优化机会：预处理、后处理、编码解码

- [x] **双 Token 调度**
  - [x] t token（文本 token）：LLM 输出
  - [x] z token（audio latent token）：语音 codec token
  - [x] mt1z 调度特性：混合 token 的调度策略
  - [x] tz 合并方案：如何统一调度两种 token
...
- [x] **Duplex 系统特殊分析**
  - [x] 双工并行带来的额外开销分析
  - [x] s2t 和 tts 同时执行时的 GPU 争用
  - [x] Session 切换的延迟开销

### 论文阅读

- [x] **Qwen3-Omni 技术报告** — 理解 Thinker-Talker 架构、MTP 机制、流式 codec 生成设计
- [x] **SoundStream** (Zeghidour et al., 2021) — 端到端神经音频 codec，理解 codec token 的生成方式
- [x] **EnCodec** (Défossez et al., 2022) — 高保真神经音频压缩，理解残差向量量化（RVQ）
- [x] **VALL-E** (Wang et al., 2023) — 基于 codec token 的语音合成范式，理解 AR + NAR 混合生成
- [x] **CosyVoice** (Du et al., 2024) — 阿里流式语音合成系统，理解 chunk-wise 流式生成策略
- [x] **Duplex TTS 相关论文** — 理解双工语音合成的最新进展
...
- [x] **Duplex 状态机**
  - [x] Listening → Thinking → Speaking 状态流转
  - [x] 状态切换的触发条件（VAD、静音检测、用户打断）
  - [x] 并发状态处理：同时收听和说话

### 设计与实现

- [x] **语音生成 stage 分离流水线优化**
...
- [x] **Duplex 双工并行优化**
  - [x] s2t 和 tts 的流水线重叠方案
  - [x] 双工场景下的 GPU 资源分配策略
  - [x] Session 级别的流水线调度

- [x] **双 Token 调度实现**
  - [x] t token 和 z token 的统一调度器设计
  - [x] mt1z 调度算法实现
  - [x] tz 合并的性能优化

### 论文阅读

- [x] **CosyVoice 2** (Du et al., 2025) — 流式语音合成的最新进展，chunk-aware 块调度
- [x] **F5-TTS** (Chen et al., 2024) — 基于 DiT 的流式 TTS，理解非自回归流式生成
- [x] **MELLE** (Zhang et al., 2024) — 连续语音生成，理解 continuous codec 生成范式
- [x] **Streaming LLM inference with pipeline parallelism** — 理解多 stage 流水线并行的通用设计模式
- [x] **Orca** (Yu et al., 2022) — 连续批处理（continuous batching）的开创性工作，理解 chunk-level 调度思想
- [x] **Duplex Dialogue Systems** — 双工对话系统的最新研究
...
- [x] **Duplex Session 管理**
  - [x] 双工 session 的 KVCache 特殊需求
  - [x] s2t 和 tts 共享的上下文信息
  - [x] Session 切换时的缓存迁移策略

### 设计与实现

- [x] **Session 级 KVCache 持久化**
...
- [x] **Duplex 特化缓存**
  - [x] 双工 session 的专用缓存池
  - [x] s2t 和 tts 的缓存隔离与共享
  - [x] 预测性缓存预热（基于对话模式）

### 论文阅读

- [x] **PagedAttention** (Kwon et al., 2023) — vLLM 的核心 KVCache 管理机制
- [x] **Prefix Caching / Prompt Caching** — 理解 prefix matching 和 KVCache 复用
- [x] **ChunkAttention** (Ye et al., 2024) — 前缀树结构的 KVCache 管理，适合多 session 场景
- [x] **RadixAttention** (Zheng et al., 2023) — SGLang 的 KVCache 复用方案
- [x] **Attention Sink** (Xiao et al., 2023) — 理解流式场景下 KVCache 的截断与保留策略
...
- [x] **双 Token 调度系统整合**
  - [x] t token 和 z token 的统一调度框架
  - [x] mt1z 调度策略的性能验证
  - [x] tz 合并的端到端优化效果

### 论文阅读

- [x] **StreamSpeech** (Ma et al., 2024) — 流式语音到语音翻译，理解 chunk-level 同声传译
- [x] **SpeechGPT** (Zhang et al., 2023) — 原生语音 LLM，理解 codec token 作为 LLM 输出的设计
- [x] **VALL-E X** (Zhang et al., 2023) — 跨语言 codec 语音合成
- [x] **实时推理系统相关**：Sarathi-Serve、DistServe、Splitwise 等论文，理解 disaggregated serving 的系统设计
- [x] **Duplex Dialogue 最新进展** — 双工对话系统的最新研究成果
...
- [x] vLLM-omni 仓库更新（关注 async_chunk、stage separation、Duplex 相关 PR）
- [x] Qwen 系列模型更新（Qwen3-Omni 后续版本）
- [x] MiniCPM 系列更新（MiniCPM-o 4.5+ Duplex 增强）
- [x] 流式语音生成领域新论文（Codec-based TTS、Streaming LLM Speech、Duplex Dialogue）
- [x] NVIDIA GTC / NeurIPS / ICML 中实时推理相关的 workshop 和论文
- [x] Omni 社区 RFC 讨论（特别是 #3745 及相关 issue）
